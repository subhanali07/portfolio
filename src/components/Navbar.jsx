...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="w-full fixed top-0 left-0 z-50 2xl:h-30  bg-[#FF9F5A] [@media(min-width:2eseseses)]:h-40 border-b-[6px] shadow-[4px_4px_0px_0px_ink]  border-ink ">
        <div className="max-w-7xl 2xl:max-w-full  mx-auto flex [@media(min-width:2eseseses)]:px-16 items-center justify-between px-6   ">
          <a href="#home" onClick={() => setIsOpen(false)}>
            <img src={salogoblackapplefont} alt="logo" className="h-18 2xl:h-28 2xl:pl-9 2xl:w-auto w-auto [@media(min-width:2eseseses)]:h-36 " />
          </a>
          <div className="hidden md:flex items-center [@media(min-width:2eseseses)]:gap-12 gap-8">
            {links.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="group relative  font-hand font-bold 2xl:text-lg text-ink bg-paper border-2 2xl:border-4 2xl:p-2 2xl:px-5  border-ink px-4 py-1.5 rounded-lg shadow-[3px_3px_0px_0px_#1A1A1A] 2xl:shadow-[6px_6px_0px_0px_#1A1A1A] [@media(min-width:2eseseses)]:active:shadow-none [@media(min-width:2eseseses)]:px-8 [@media(min-width:2eseseses)]:py-2 2xl:rounded-2xl [@media(min-width:2eseseses)]:hover:shadow-[4px_4px_0px_0px_#1A1A1A] [@media(min-width:2eseseses)]:shadow-[7px_7px_0px_0px_#1A1A1A] transition-all duration-100 ease-in-out hover:translate-x-px 2xl:hover:translate-x-0.5 2xl:hover:translate-y-0.5 hover:translate-y-px hover:shadow-[2px_2px_0px_0px_#1A1A1A] 2xl:hover:shadow-[4px_4px_0px_0px_#1A1A1A] active:translate-x-0.75 2xl:active:translate-x-1 active:translate-y-0.75 2xl:active:translate-y-1 active:shadow-none 2xl:active:shadow-none block"
              >
